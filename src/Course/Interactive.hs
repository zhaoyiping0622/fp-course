...
reverseInteractive = 
  do openFile <- getLine 
     wf <- getLine
     content <- readFile openFile
     let reversedContent = reverse content
     writeFile wf reversedContent
...
encodeInteractive =
  do s <- getLine
     let out = foldRight (\a b -> if a == ' ' then "%20"++b else if a == '\t' then "%09"++b else if a == '\"' then "%22"++b else a:.b) Nil s
     putStrLn out
