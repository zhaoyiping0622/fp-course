...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<$>) f a = StateT (\s -> (\(a',s) -> (f a', s)) <$> runStateT a s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) f a = StateT (\s -> runStateT f s >>= (\(f', s') -> runStateT (f' <$> a) s'))
    -- error "todo: Course.StateT (<*>)#instance (StateT s k)"
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (=<<) f a = StateT (\s -> (\(a',s') -> runStateT (f a') s') =<< runStateT a s)
    -- error "todo: Course.StateT (=<<)#instance (StateT s k)"
...
distinct' ::
  ...
  List a
  -> List a
distinct' xs = eval' (filtering (\x -> state' (\s -> (not $ S.member x s, S.insert x s))) xs) S.empty
...
distinctF xs = evalT (filtering 
  (\x -> 
    StateT (\s -> (if x > 100 then Empty else Full(not $ S.member x s, S.insert x s)))
  ) xs) S.empty
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<*>) f a = OptionalT (runOptionalT f >>= onFull (\f' -> (f' <$>) <$> runOptionalT a))
    -- f' :: Optional (a->b)
    -- a' :: Optional a
    -- error "todo: Course.StateT (<*>)#instance (OptionalT k)"
...
instance Applicative (Logger l) where
  ...
  (<*>) ::
    ...
    -> Logger l a
    -> Logger l b
  (<*>) (Logger a f) (Logger b x) = Logger (a ++ b) (f x)
    -- error "todo: Course.StateT (<*>)#instance (Logger l)"
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  (=<<) f (Logger ls x) = Logger (ls ++ ls') y
    where (Logger ls' y) = f x
...
distinctG xs = runOptionalT $ reverse . fst <$> foldLeft (\b a -> b >>= checkLogger a) (OptionalT $ emptyLogger $ Full (Nil, S.empty)) xs
  -- error "todo: Course.StateT#distinctG"

checkLogger :: (Integral a, Show a) => a -> (List a, S.Set a) -> OptionalT (Logger Chars) (List a, S.Set a)
checkLogger x (l, s) = OptionalT (if x > 100 then log1 (abortingNumber x) Empty
      else (if even x then log1 (evenNumber x) else emptyLogger) 
        (if S.member x s then pure (l, s) else pure (x :. l, S.insert x s)))

emptyLogger :: a -> Logger l a
emptyLogger = Logger Nil

evenNumber :: Show a => a -> Chars
evenNumber x = "even number: " ++ show' x

abortingNumber :: Show a => a -> Chars
abortingNumber x = "aborting > 100: " ++ show' x
