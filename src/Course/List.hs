...
seqOptional ((Full x):.xs) = let t = seqOptional xs in case t of 
  Empty -> Empty
  Full p -> Full $ x:.p
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 x = lengthGT4' x 5
  where
    lengthGT4' _ 0 = True
    lengthGT4' Nil n = False
    lengthGT4' (_:.xs) n = lengthGT4' xs (n-1)
