...
jsonString = betweenCharTok '"' '"' $ 
  list (
    (is '\\' *> 
      ((character >>= (\c -> case fromSpecialCharacter <$> toSpecialCharacter c of Full x -> pure x; _ -> unexpectedCharParser c))
    ||| hexu)) -- read special characters
    ||| satisfyAll ((/='\\'):.(/='"'):.Nil))
...
jsonNumber = (\x -> 
  case readFloat x of 
    Full x' -> x'
    _ -> error $ show $ "shit " ++ show' x) <$> 
  (jsonNumber1 .++. optionalListParser jsonNumber2 .++. optionalListParser jsonNumber3)
  -- error "todo: Course.JsonParser#jsonNumber"

jsonNumber1 ::
  Parser Chars
jsonNumber1 = optionalChar '-' .++. ((is '0' .:. pure Nil) ||| (satisfyAll ((/='0'):.isDigit:.Nil) .:. list digit))

jsonNumber2 ::
  Parser Chars
jsonNumber2 = is '.' .:. list digit

jsonNumber3 ::
  Parser Chars
jsonNumber3 = (is 'e' ||| is 'E') .:. optionalListParsers (optionalChar '-' :. optionalChar '+' :. Nil) .++. list1 digit
...
jsonObject ::
  Parser Assoc
jsonObject = spaces *> betweenSepbyComma '{' '}' ((,) <$ spaces <*> jsonString <* spaces <* is ':' <*> jsonValue) <* spaces
...
jsonValue ::
  Parser JsonValue
jsonValue = spaces *> ((JsonString <$> jsonString)
  ||| (JsonRational <$> jsonNumber)
  ||| (JsonObject <$> jsonObject)
  ||| (JsonArray <$> jsonArray)
  ||| (JsonTrue <$ jsonTrue)
  ||| (JsonFalse <$ jsonFalse)
  ||| (JsonNull <$ jsonNull)) <* spaces
