...
changeErrorResult UnexpectedEof = UnexpectedEof
changeErrorResult (ExpectedEof i) = ExpectedEof i
changeErrorResult (UnexpectedChar c) = UnexpectedChar c
changeErrorResult (UnexpectedString s) = UnexpectedString s
changeErrorResult (Result _ _) = error "only error result can be changed" 
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f a = P (\s -> case parse a s of 
    Result i x -> parse (f x) i
    x -> changeErrorResult x)
    -- error "todo: Course.Parser (=<<)#instance Parser"
...
list ::
  Parser a
  -> Parser (List a)
list x = list1 x ||| valueParser Nil
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\a b -> (:.) <$> a <*> b) (valueParser Nil)
...
smokerParser ::
  Parser Bool
smokerParser = (is 'y' ||| is 'n') >>= (\c -> valueParser (c=='y'))
